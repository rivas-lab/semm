#' Fit alternate (no sex-specific variants) model to the data. This is used
#' for comparison with model2 to test for evidence for sex-specific components.
#'
#' @export
...
#'
model2alt_stan <- function(dat=NULL, B=NULL, SE=NULL, ...){
  ...
  # fit the model
  standata <- list(B=dat$B, SE=dat$SE, N=nrow(dat$B), M = ncol(dat$B), K = 2)
  out <- rstan::sampling(stanmodels$model2alt, data = standata, ...)
  return(out)
}
