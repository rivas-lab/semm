...
#' Check the input parameters for `prep_gwas_input()`
#'
#' @param gwas.files list of files containing GWAS input
#' @param cat.labels vector of labels for the files specifying their categories
#' @param se.cut the max standard error allowed
.check_read_in_params <- function(gwas.files, cat.labels, se.cut){
  ...
  if (!is.null(cat.labels)){
    .my_assert("please provide the same number of labels as files",
               length(cat.labels) == ndim)
  }
...
#'
#' @param dat the data frame resulting from reading in the gwas file
#' @param B name of column with betas
#' @param SE name of column with ses
#' @param ID name of column with variant dds
#' @param CHR name of column with chromosome number
#' @param P name of column name with p-vals
.check_gwas_f_format <- function(dat, ID, B, SE, CHR, P){
  .my_assert("GWAS file has insufficient columns and/or rows",
             nrow(dat) > 1 & ncol(dat) >= 3)
  .my_assert("the required columns are not present, please rename or specify ID, BETA, and SE columns",
             length(setdiff(c(ID, B, SE), colnames(dat)))==0)

  .my_warn("p-value and chromosome columns were not included",
           length(setdiff(c(P, CHR), colnames(dat)))==0)
}

#' Check that the variants to keep ids overlap with the data ids.
#'
#' @param var.keep list of variants to keep
#' @param data.ids list of variant ids from the data
...
#' Check that the final list of overlapping variants is non-zero and warn
#' if it is small.
#'
#' @param rows.to.keep list of variants after filtering
...
#' Check a data.frame is all numeric
#'
#' @param df a dataframe
#' @return TRUE if all values in a data frame are numeric
.all_numeric <- function(df){
  all(apply(df, c(1,2), is.numeric))
}

#' Check the input data is formatted correctly
#'
#' @param dat input data for STAN, should be generated by `prep_gwas_input()`
...
#' Check a posterior df is formatted correctly
#'
#' @param posterior.df a data frame with posteriors
